...
HAPError HandleLightBulbOnRead(
    ...
    *value = accessoryConfiguration.state.lightBulbOn;
    HAPLog(&kHAPLog_Default, "%s: %s", __func__, *value ? "true" : "false");
...
HAPError HandleLightBulbBrightnessRead(
    ...
    *value = accessoryConfiguration.state.lightBulbBrightness;
    HAPLog(&kHAPLog_Default, "%s: %d", __func__, *value);
...
HAPError HandleLightBulbHueRead(
    ...
    *value = accessoryConfiguration.state.lightBulbHue;
    HAPLog(&kHAPLog_Default, "%s: %g", __func__, *value);
...
HAPError HandleLightBulbOnWrite(
    ...
    if (accessoryConfiguration.state.lightBulbOn != value) {
        ...
        SaveAccessoryState();
        struct led_color_t color_to_send = { 0U, 0U, 0U};
        if (value) {
            HAPError err = GetRGBFromStore(&color_to_send);
            if (kHAPError_None != err) { return err; }
        }
        xQueueSend(color_queue, (void*)&color_to_send, 100);
...
HAPError HandleLightBulbBrightnessWrite(
    ...
    if (accessoryConfiguration.state.lightBulbBrightness != value) {
        ...
        struct led_color_t color_to_send = {0U, 0U, 0U};
        GetRGBFromStore(&color_to_send);
        xQueueSend(color_queue, (void*)&color_to_send, 100);
        HAPLog(&kHAPLog_Default, "%s: (R:%d G:%d B:%d)", __func__, color_to_send.red, color_to_send.green, color_to_send.blue);
...
HAPError GetRGBFromStore(struct led_color_t* current_color) {
    led_strip_hsv2rgb((uint8_t) accessoryConfiguration.state.lightBulbBrightness,
                        (uint8_t) accessoryConfiguration.state.lightBulbSaturation, 
                        (uint16_t) accessoryConfiguration.state.lightBulbHue, 
                        &((*current_color).red),
                        &((*current_color).green),
                        &((*current_color).blue));
    return kHAPError_None;
